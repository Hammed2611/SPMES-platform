...
export const downloadProjectPDF = (project, student, lecturer) => {
  try {
    ...
    const transcriptId = project?.id ? `TR-${project.id.slice(-6).toUpperCase()}` : 'TR-XXXXXX';
    doc.text(`Transcript ID: ${transcriptId}`, 140, 60);
    ...
    const infoData = [
      ['Student Name:', student?.name || 'N/A', 'Matric Number:', student?.matricNumber || 'N/A'],
      ['Department:', student?.department || 'N/A', 'Category:', project?.category || 'N/A'],
      ['Project Title:', { content: project?.title || 'N/A', colSpan: 3 }],
    ...
    if (project?.rubric) {
      ...
      rubricRows.push(['N/A', 'N/A', 'N/A', 'No rubric data found.']);
    }
    ...
    // Download
    const fileName = student?.name ? `Result_${student.name.replace(/\s+/g, '_')}.pdf` : 'Project_Result.pdf';
    doc.save(fileName);
  } catch (error) {
